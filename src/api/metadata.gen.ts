 ...
 * URL:
 */
export const useMetadataApiV1QueryParameterList = (
    params: RequestParams = {},
    autoExecute = true,
    overwriteWorkspace: string | undefined = undefined
) => {
    ...
    const [state, setState] =
        useState<IuseMetadataApiV1QueryParameterListState>({
            isLoading: true,
            isExecuted: false,
        })
    const [lastInput, setLastInput] = useState<string>(
    ...
    const sendRequest = (
        ...
        try {
            ...
            const reqparamsSignal = { ...reqparams, signal: abortCtrl.signal }
            api.metadata
                .apiV1QueryParameterList(reqparamsSignal)
                .then((resp) => {
                    setState({
                        ...state,
                        error: undefined,
...
interface IuseMetadataApiV1QueryParameterCreateState {
    isLoading: boolean
    isExecuted: boolean
    response?: void
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
...
export const useMetadataApiV1QueryParameterCreate = (
    ...
    const [state, setState] =
        useState<IuseMetadataApiV1QueryParameterCreateState>({
            isLoading: true,
            isExecuted: false,
        })
    const [lastInput, setLastInput] = useState<string>(
    ...
    const sendRequest = (
        ...
        try {
            ...
            const reqparamsSignal = { ...reqparams, signal: abortCtrl.signal }
            api.metadata
                .apiV1QueryParameterCreate(reqreq, reqparamsSignal)
                .then((resp) => {
                    setState({
                        ...state,
                        error: undefined,
