...
export const trendChart = (
    ...
    for (let i = 0; i < trend?.length; i += 1) {
        ...
        if (chart === 'aggregated') {
            data.push((trend[i]?.count || 0) + (data?.at(i - 1) || 0))
        } else {
            data.push(trend[i]?.count || 0)
        }
...
export const buildTrend = (
    apiResp: GithubComKaytuIoKaytuEnginePkgInventoryApiResourceTypeTrendDatapoint[],
    chart: 'trending' | 'aggregated',
    granularity: 'monthly' | 'daily' | 'yearly',
    topN: number
) => {
